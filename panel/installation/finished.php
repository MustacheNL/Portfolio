<?php
require '../../private/config.php';
?>
<html>
    <head>
        <title>
            Installatie
        </title>
        <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
    </head>

    <style>
        .m-t-10 {
            margin-top: 10px;
        }

        .panel-primary {
            margin: auto;
            margin-top: 50%;
        }
    </style>

    <body>
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-md-offset-3">
                    <div class="panel panel-primary">
                        <div class="panel-heading">Installatie: Klaar</div>
                        <div class="panel-body">
                            <h2>Feest!</h2><br/>


                                Het installeren is gelukt zonder dat er iets mis is gegaan, anders kwam je hier niet.
                                Bedankt voor het volgen van de snelle installatie van dit geweldige systeem, en veel
                                veel plezier met het gebruiken. Zorg dat het systeem altijd up-to-date is door regelmatig
                                je dashboard te kijken voor updates!
                            <br/><br/>
                            <center><a href="/panel/"><input type="submit" class="btn btn-primary m-t-10" name="submit" id="submitbtn" value="Afronden""></a></center>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </body>
</html>